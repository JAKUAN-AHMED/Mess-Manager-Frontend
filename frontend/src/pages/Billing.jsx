...
import { Banknote, ChevronLeft, ChevronRight, CheckCircle, AlertCircle, X, TrendingUp, Utensils, Wallet, SlidersHorizontal, Plus, Minus } from 'lucide-react';
...
export function Billing() {
  ...
  const fetchBills = async () => {
    setLoading(true);
    try {
      const billsRes = await api.get(`/reports/all-bills?month=${month}&year=${year}`);
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          {isAdmin && (
            <button onClick={() => setAdjModal(true)}
              className="flex items-center gap-1.5 px-3 py-2 bg-violet-50 hover:bg-violet-100 text-violet-700 border border-violet-200 rounded-xl text-sm font-semibold transition-all">
